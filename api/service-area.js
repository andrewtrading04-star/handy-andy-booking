...
export default async function handler(req, res) {
  ...
  // TEMPORARY read-only diagnostic — dump the full service price-adjustment rules. Remove after use.
  if (req.method === 'GET' && req.query.debug === 'adjraw') {
    if (req.query.token !== 'adj-7q2x') return res.status(403).json({ error: 'forbidden' });
    try {
      const r = await fetch('https://api.zenbooker.com/v1/services/1685657518404x705274829881212200', { headers: { Authorization: `Bearer ${process.env.ZENBOOKER_API_KEY}` } });
      const sj = await r.json().catch(() => ({}));
      return res.status(200).json({ pricing_method: sj.pricing_method, base_price: sj.base_price, min_price: sj.min_price, territory_price_adjustments: sj.territory_price_adjustments || null });
