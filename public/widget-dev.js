/**
 * Handy Andy TV Mounting — Booking Widget  ·  DEV / SANDBOX COPY
 * ----------------------------------------------------------------------------
 * This is an EXACT copy of the live `widget.js`, made so we can safely change
 * and test the booking flow WITHOUT touching the version customers use.
 *
 *   • The live site embeds `widget.js` — that file is the production widget and
 *     must NOT be edited until the new system is 100% ready to switch over.
 *   • Edit THIS file (`widget-dev.js`) for any experiments. Nothing on the live
 *     site loads it, so changes here cannot affect real customer bookings.
 *
 * NOTE: this copy still points at the production API_BASE below, so submitting
 * a booking from it would create a REAL job. Before live testing, point it at a
 * test endpoint (or use a test territory) so sandbox bookings stay isolated.
 */
(function () {
  'use strict';
